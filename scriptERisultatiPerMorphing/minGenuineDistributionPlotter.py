...
# Calcola distanza del punteggio minimo per ogni gruppo
for key, scores in dati_raggruppati.items():
    ...
    idx_min = scores.index(min_score)
    distanza_score_min = x_vals[idx_min]
    punti.append((distanza_score_min, min_score))
...
plt.title('MIN SCORE DISTRIBUTION WITH RESPECT TO DISTANCE')
plt.xlim(0, 850)
plt.grid(axis='x')
plt.tight_layout()

# Salva e mostra il grafico
plt.savefig('retta_score_minimi.png')
plt.show()

print("Grafico su retta salvato come 'retta_score_minimi.png'.")
