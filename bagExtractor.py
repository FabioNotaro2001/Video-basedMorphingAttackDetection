...
def get_intrinsics(bag_filename):
...
# es. "./res/bag_files/Acquisizioni_Raw/20230908_160009.bag"
def extract_frames_from_bag(bag_filename):
    ...
    unaligned_color_folder = f"./res/frames/{bag_name}/rgb_unaligned/"
    aligned_color_folder = f"./res/frames/{bag_name}/rgb_aligned/"
    depth_16bit_folder = f"./res/frames/{bag_name}/depth_16bit/"
    depth_8bit_folder = f"./res/frames/{bag_name}/depth_8bit/"
    infrared_folder = f"./res/frames/{bag_name}/infrared/"
    ...
    try:
        while True:
            ...
            if(unaligned_rgb_frame):
                ...
                unaligned_color_image = cv2.cvtColor(unaligned_color_image, cv2.COLOR_RGB2BGR)
                unaligned_color_frame_filename = os.path.join(
                    unaligned_color_folder, f"{frame_count:06d}.jpg"
                )
                cv2.imwrite(unaligned_color_frame_filename, unaligned_color_image)
            ...
            if(color_frame):
                ...
                cv2.imwrite(color_frame_filename, color_image)
...
def extract_bag_from_folder(folder_path):
    bag_paths = [bag for bag in os.listdir(folder_path) if bag.endswith(".bag")]
    for bag in bag_paths:
        filename = bag.split('.')[0]
        if filename not in os.listdir("./res/frames"):
            bag_full_path = os.path.join(folder_path, bag)
            print(f"Extracting {filename} ....")
            extract_frames_from_bag(bag_full_path)
            
            # Rimuove il file .bag dopo la sua elaborazione
            try:
                os.remove(bag_full_path)
                print(f"Deleted {bag_full_path} to free up space.")
            except Exception as e:
                print(f"Error deleting {bag_full_path}: {e}")

def main():
    #extract_frames_from_bag("20250203_132654.bag")  # Estrazione singolo file

    # Estrazione da folder con eliminazione progressiva
    extract_bag_from_folder("test")
