...
def aggrega_distanze(input_file, output_file):
    ...
    aggregated_df = df.groupby(["ID_SOGGETTO", "SEQUENZA", "POSA"])['DISTANZA_COSENO'].agg(['mean', 'max', 'min']).reset_index()
...
if __name__ == "__main__":
    ...
    input_file = os.path.join(dataset_root, "distanze_coseno.txt")
    output_file = os.path.join(dataset_root, "distanze_coseno_aggregato.txt")
