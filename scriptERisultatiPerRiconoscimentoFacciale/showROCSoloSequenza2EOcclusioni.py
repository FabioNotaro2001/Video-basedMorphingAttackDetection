...
def read_and_extract_values(file_path: str) -> np.ndarray:
    ...
    for line in lines:
        ...
        if len(columns) >= 5:
            quintultima = columns[-5]
            quartultima = columns[-4]
            
            # Verifica entrambe le condizioni
            if (quintultima in ('2', '02')) and (quartultima == 'looking_around_with_occlusion'):
                # Aggiungi il valore della terzultima colonna
                values.append(columns[-1])  # Terzultima colonna

    # Converti la lista in un array numpy
    filtered = np.array(values, dtype=float)
    return filtered
...
plt.title('DET CURVE RELATED TO MIN SCORES')
